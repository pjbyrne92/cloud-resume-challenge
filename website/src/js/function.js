...
AWS.config.update({ region: 'eu-west-1' }); // Update the region as per your configuration
var lambda = new AWS.Lambda();

lambda.invoke({
  FunctionName: 'cloud-resume-challenge-api ',
  Payload: '{}'
}, function(err, data) {
  if (err) {
    console.log(err);
  } else {
    var visitorCount = JSON.parse(data.Payload);
